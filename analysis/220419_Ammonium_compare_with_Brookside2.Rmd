---
title: "220419_Ammonium_compare_with_Brookside2"
...
## 220419 Ammonium compare with Brookside one more time
Researcher: **Kiseok Lee** \
Experiment Date: 4/18/22 \
Analysis Date: 4/19/22
Lab: **Seppe Kuehn**
...
df_220419 <- read.xlsx("data/220419_Ammonia_sample_plate0.xlsx")
...
df_ave_220419 <- df_220419 %>% group_by(Sample) %>% summarize(`NH4-N_ppm_mine` = mean(Ammonia_ppm)*0.1)

df_merged <- df_brook %>% filter(Sample_type == "Slurry") %>%  select(Sample, `NH4-N_ppm`) %>% left_join(df_ave_220419, by=c("Sample"="Sample"))
...
ggplot(df_melt, aes(x=Sample, y=value, fill= Measurement)) +
  ...
  ggtitle("NH4 measurement comparison (220419 Assay of stored frozen sample) \n") +
  geom_text(aes(label = round(value,3)), size = 3, vjust = -1.5, family="serif", show.legend = FALSE, position = position_dodge(0.8))+
  mytheme_2d 

# agreement

ggplot(df_merged, aes(x=`NH4-N_ppm_mine`, y=`NH4-N_ppm`)) +
