---
title: "221015_Griess_Cook_before_wetting"
...
## Cook before wetting (BW) plate1,2 (soil1, 5, 10, 14) (Griess assay)
Researcher: **Kiseok Lee** \
Experiment Date: 10/3/22 - 10/7/22 (5 days) \
...
for (i in vec_sample){
  for (j in vec_time){
    ...
    print(platename)
    # import files
    assign(platename, openxlsx::read.xlsx(paste0("data/221015_Griess_",timepoint,".xlsx")))
...
# assign(vec_platename[1], openxlsx::read.xlsx(paste0("data/221015_Griess_",vec_timepoint[1],".xlsx")))
df_BW_plate1_2_T0
df_BW_plate1_2_T1


head(df_BW_plate1_2_T0)
colnames(df_BW_plate1_2_T0)
dim(df_BW_plate1_2_T0)
...
df_BW_plate1_2_T4 %<>% filter(!(Well %in% c("B04"))) # Switched wells by mistake
...
Time_table <- openxlsx::read.xlsx("data/221015_time_table.xlsx")
# Time_table_BN <- openxlsx::read.xlsx("data/221015_time_table_BN.xlsx")
...
dim(df_kyle)
# write.xlsx(df_kyle, "221015_Griess_antibiotics_experiment_to_kyle.xlsx")

df_kyle2 <- df_kyle %>% filter(Titration_type %in% c("NaOH", "HCl"))
dim(df_kyle2)
# write.xlsx(df_kyle2, "221015_Griess_2soil_pH7_2soil_pH5_remove_blanks.xlsx")
...
df_221016_pH <- read.xlsx("data/221016_pH_Cook_BW_plate1_2.xlsx") %>% select(-H_mM)
df_221016_pH$Time_point # T9
dim(df_221016_pH)
colnames(df_221016_pH)
df_221016_pH %>% unique() %>% nrow

df_pH2 <- df_221016_pH %>% select(Soil, Titration_type, Unit, Chloramphenicol, Cycloheximide, pH) 
...
# df_pH_label <- df_221016_pH %>% group_by(Titration_type, Unit) %>% summarize(pH_min = round(min(pH), 2), pH_max = round(max(pH), 2))
...
write.xlsx(df_plot_pH, "221015_Griess_Cook_before_wetting_water.xlsx")
...
<!-- ```{r} -->
<!-- df_ammo_plot3 <- read.xlsx("data/221015_Ammonia_sterile.xlsx") -->
...
<!--   # label -->
<!--   # geom_text(aes(label = round(NH4_mM,3)), size = 3, vjust = -1.5, show.legend = FALSE)+ -->
<!--   mytheme_2d + -->
<!--   facet_grid(. ~ Soil) + -->
<!--   theme(strip.background = element_rect(colour="black", fill="white", size=0.1))+ -->
<!--   theme(strip.text.x = element_text(size = 17)) -->
...
<!-- # write.xlsx(df_reduced, "221015_nitrate_reduced_autoclaved_samples.xlsx") -->
...
<!-- df_a_bcf <- read.xlsx("data/221015_Ammonia_df_a_bcf.xlsx") -->
...
<!-- # write.xlsx(df_kyle2, "221015_Griess_antibiotics_experiment_to_kyle_w_autoclaved.xlsx") -->
...
<!-- ```{r} -->
<!-- df_kyle2 <- read.xlsx("data/221015_Griess_antibiotics_experiment_to_kyle_remove_blanks_add_pH.xlsx") -->
