...
df_T %<>% mutate(Ammonia_ppm = Ammonia_ppm * (5/2))
head(df_T)
...
df_NH4 <- df_T %>% group_by(Nitrite_input, Nitrate_input, Soil, Titration_type, Concentration_M, Added_ul, Sample_type, Time_point, Time_minutes, Time_hours, Time_days) %>% summarise(Ave_Ammonia_ppm = mean(Ammonia_ppm), Std_Ammonia_ppm = sd(Ammonia_ppm), Ave_NH4_mM = mean(NH4_mM), Std_NH4_mM = sd(NH4_mM)) %>% ungroup()
...
# test
df_pH <- df_NH4_mcf %>% filter(Titration_type %in% c("NaOH","HCl"))
dim(df_pH)
Added_Volume <- 1.7 # ml
moisture_percent_1 = 19 # NicholsC
moisture_percent_2 = 33.2 # Allandale
...
ggplot(df_pH, aes(x=Time_hours, y=Ave_NH4_mM, color=H_mM, group=H_mM)) +
