---
title: "220421_Ammonia_antibiotics_and_sterile"
...
## Antibiotics and sterile autoclaved soil (Ammonia assay - Salicylate-Hypochloride assay)
Researcher: **Kiseok Lee** \
Experiment Date: 4/14/22 - 4/18/22 (5 days) \
Analysis Date: 4/25/21
Lab: **Seppe Kuehn**
...
# bind two dataframe
df_a <- rbind(df_plate1, df_plate2, df_plate3, df_plate4, df_plate5, df_plate6, df_plate7, df_plate8, df_plate9, df_plate10, df_plate11)
colnames(df_a)
...
colnames(df_a)
# time_point order
df_a$Time_point <-  factor(df_a$Time_point, levels = c(paste0("LBA_T",0:10), paste0("Sterile_T",0:10)))
...
df_a$Volume <- ifelse(df_a$Soil == "LaBaghWoods_pH6.66", Added_Volume + Soil_mg*(moisture_percent_1/100),
                      ifelse(df_a$Soil == "LaBaghWoods_autoclaved_pH6.66", Added_Volume + Soil_mg*(moisture_percent_2/100),
                             0))
...
df_a %>% filter(Soil == "LaBaghWoods_autoclaved_pH6.66")%>% select(H_mM) %>% unique() %>% arrange(H_mM)
colnames(df_a)
...
df_a_mcf_1 <- df_a %>% filter(Soil == "LaBaghWoods_pH6.66") %>% filter(Titration_type %in% c("NaOH","HCl","No_Nitrate")) %>% mutate(NH4_mM = NH4_mM * mcf_1)
dim(df_a_mcf_1) #423
df_a_mcf_2 <- df_a %>% filter(Soil == "LaBaghWoods_autoclaved_pH6.66") %>%  filter(Titration_type %in% c("NaOH","HCl","No_Nitrate")) %>% mutate(NH4_mM = NH4_mM * mcf_2)
dim(df_a_mcf_2) #426
# df_a_mcf_3 <- df_a %>% filter(Soil == "Crerar1_pH8.05") %>% filter(Titration_type %in% c("NaOH","HCl","No_Nitrate")) %>% mutate(NO3_mM = NO3_mM * mcf_3, NO2_mM = NO2_mM * mcf_3)
# dim(df_a_mcf_3) #426
...
df_a_mcf <- rbind(df_a_mcf_1, df_a_mcf_2, df_a_others_mcf)
dim(df_a_mcf) # 1046
...
write.xlsx(df_a_bcf, "220421_Ammonia_df_a_bcf.xlsx")
```
...
df_plot <- df_a_pH %>% filter(Antibiotics_type == "None") %>% select(H_mM, NH4_mM, Soil, Titration_type, Unit, Antibiotics_type, Time_hours) 
df_plot %>% filter(H_mM == 0) %>% dim()
df_plot %>% filter(H_mM == 0) %>% group_by(H_mM, Soil, Time_hours) %>% summarize(NH4_mM = mean(NH4_mM)) %>% ungroup()
df_sub <- df_plot %>% filter(H_mM == 0) %>% group_by(H_mM, Soil, Titration_type, Unit, Antibiotics_type, Time_hours) %>% summarize(NH4_mM = mean(NH4_mM)) %>% ungroup()
...
ggplot(df_plot2 %>% filter(Soil == "LaBaghWoods_autoclaved_pH6.66"), aes(x=Time_hours, y=NH4_mM, color=H_mM, group=H_mM)) +
  geom_point(size=2.5, shape=16) +
  geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NH4_mM - Std_NH4_mM, ymax=NH4_mM + Std_NH4_mM), width=.05)+
  scale_colour_gradientn(colours = col_pH(100)) +
...
df_plot3 <- df_plot2 %>% left_join(df_220422_pH, by=c("Soil"="Soil", "Titration_type"="Titration_type","Unit"="Unit", "Antibiotics_type"="Antibiotics_type"))
df_plot3$Soil <-  factor(df_plot3$Soil, levels = c("LaBaghWoods_pH6.66","LaBaghWoods_autoclaved_pH6.66"))


ggplot(df_plot3, aes(x=Time_hours, y=NH4_mM, color=pH, group=pH)) +
  geom_point(size=2.5, shape=16) +
  geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NH4_mM - Std_NH4_mM, ymax=NH4_mM + Std_NH4_mM), width=.05)+
...
write.xlsx(df_plot3, "220421_Ammonia_sterile.xlsx")
```

See samples that have undergone reduction

```{r}
df_reduced <- read.xlsx("data/220421_nitrate_reduced_autoclaved_samples.xlsx")
colnames(df_reduced)
df_plot_au <- df_plot3 %>% filter(Soil == "LaBaghWoods_autoclaved_pH6.66")


df_reduced2 <- df_reduced %>% select("Titration_type", "Unit","Nitrate_reduced")

df_plot_au %<>% left_join(df_reduced2, by=c("Titration_type"="Titration_type","Unit"="Unit"))
# remove NA
colnames(df_plot_au)
df_plot_au %>% filter(is.na(Nitrate_reduced))
df_plot_au %>% filter(Titration_type == "HCl", Unit == 11)

df_plot_au <- df_plot_au[complete.cases(df_plot_au), ]
df_plot_au$Nitrate_reduced <- ifelse(df_plot_au$Nitrate_reduced ==T, "NO3 fully reduced", "Not reduced")

#
df_plot_au$Nitrate_reduced <- factor(df_plot_au$Nitrate_reduced, levels=c("Not reduced","NO3 fully reduced"))

ggplot(df_plot_au, aes(x=Time_hours, y=NH4_mM, color=pH, group=pH)) +
  geom_point(size=2.5, shape=16) +
  geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NH4_mM - Std_NH4_mM, ymax=NH4_mM + Std_NH4_mM), width=.05)+
  scale_colour_gradientn(colours = rev(col_pH(100))) +
...
df_au_T0 <- df_plot_au %>% filter(Time_hours == 0)

ggplot(df_au_T0, aes(x=H_mM, y=NH4_mM, color=H_mM, group=H_mM)) +
  geom_point(size=2.5, shape=16) +
  geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NH4_mM - Std_NH4_mM, ymax=NH4_mM + Std_NH4_mM), width=.05)+
  scale_colour_gradientn(colours = col_pH(100)) +
...
ggplot(df_au_T0, aes(x=pH, y=NH4_mM, color=pH, group=pH)) +
  geom_point(size=2.5, shape=16) +
  geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NH4_mM - Std_NH4_mM, ymax=NH4_mM + Std_NH4_mM), width=.05)+
  scale_colour_gradientn(colours = rev(col_pH(100))) +
...
## 4.2.1. Antibiotics ammonium
```{r}

# only antibiotics treated samples
df_LBA <- df_a_bcf %>% filter(Soil == "LaBaghWoods_pH6.66")
dim(df_LBA) # 521
...
df_autoclaved <- df_a_pH %>% filter(H_mM == 0) 
...
# plot
ggplot(df_effect, aes(x=Time_hours, y=NH4_mM, color=Antibiotics_type, group=Antibiotics_type)) +
  geom_point(size=2.5, shape=16) +
  # geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NO3_mM - Std_NO3_mM, ymax=NO3_mM + Std_NO3_mM), width=.05)+
  # scale_colour_gradientn(colours = col_pH(100)) +
...
colnames(df_effect)
dim(df_effect)
df_effect_ave <- df_effect %>% group_by(Nitrite_input, Nitrate_input, Soil, Titration_type, Sample_type, Antibiotics_type, H_mM, Time_point, Time_minutes, Time_hours, Time_days) %>% summarise(Std_NH4_mM = sd(NH4_mM), NH4_mM = mean(NH4_mM)) %>% ungroup()
dim(df_effect_ave)

# Ammonium
ggplot(df_effect_ave, aes(x=Time_hours, y=NH4_mM, color=Antibiotics_type, group=Antibiotics_type)) +
  geom_point(size=2.5, shape=16) +
  geom_line(size=1.2)+
  geom_errorbar(aes(ymin=NH4_mM - Std_NH4_mM, ymax=NH4_mM + Std_NH4_mM), width=.05)+
  # scale_colour_gradientn(colours = col_pH(100)) +
...
## Plot with nitrate, nitrite dynamics
```{r}
# data frame
df_with <- df_a_pH %>% filter(Soil == "LaBaghWoods_pH6.66") %>% select(H_mM, NH4_mM, Soil, Titration_type, Unit, Antibiotics_type, Time_hours) 
df_with %>% filter(H_mM == 0) %>% dim()
df_with %>% filter(H_mM == 0) %>% group_by(H_mM, Soil, Time_hours) %>% summarize(NH4_mM = mean(NH4_mM)) %>% ungroup()
df_sub <- df_with %>% filter(H_mM == 0) %>% group_by(H_mM, Soil, Titration_type, Unit, Antibiotics_type, Time_hours) %>% summarize(NH4_mM = mean(NH4_mM)) %>% ungroup()
df_main <- df_with %>% filter(H_mM != 0)
dim(df_with)
dim(df_main)
df_with2 <- rbind(df_main, df_sub)

# input pH data
df_kyle2 <- read.xlsx("data/220421_Griess_antibiotics_experiment_to_kyle_remove_blanks_add_pH.xlsx")
df_pH_effect <- df_kyle2 %>% select(Soil, Sample_type, Titration_type, Antibiotics_type, Unit,pH ) %>% group_by(Soil, Sample_type, Titration_type, Antibiotics_type, Unit) %>% summarize(pH = mean(pH))

df_with3 <- df_with2 %>% left_join(df_pH_effect, by=c("Soil"="Soil", "Titration_type"="Titration_type","Unit"="Unit", "Antibiotics_type"="Antibiotics_type"))
colnames(df_with3)

# reorder factor
df_with3$Antibiotics_type <- factor(df_with3$Antibiotics_type, levels = c("None", "CHL", "NYT", "CHL_NYT", "Autoclaved"))

# label with pH range
df_pH_label <- df_with3 %>% group_by(Titration_type, Unit) %>% summarize(pH_min = min(pH), pH_max = max(pH))
# df_pH_label$pH_max <- as.character()

df_pH_label %<>% unite(col='pH_range', c("pH_min","pH_max"), sep="-", remove=F)

df_pH_range <- df_with3 %>% left_join(df_pH_label, by =c("Titration_type"="Titration_type", "Unit"="Unit"))
dim(df_pH_range)
