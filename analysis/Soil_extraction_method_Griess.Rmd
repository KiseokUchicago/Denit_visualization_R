...
(2) 0.1g & Spindown method
(3) 1g & filtered method
(4) 0.1g & Spindown method
```{r}
...
ggplot(df_spin1, aes(x=Buffer, y=Ave_NO2_mM, color=Spike_in, group=Spike_in)) +
  ...
  ggtitle("(1) Soil weight 1g & Spindown method \n") +
  # label
  geom_text(aes(label = round(Ave_NO2_mM,3)), size = 3, vjust = -1.5, family="serif")+
  mytheme_2d
...
# (1) plot
ggplot(df_extract, aes(x=Method, y=pH, color=Solution, group=Solution)) +
  geom_point(size=2.5, shape=21) +
  geom_line(size=1)+
  scale_fill_brewer(palette='Set2') +
  ylab("pH \n") +
