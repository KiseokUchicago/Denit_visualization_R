---
title: "220602_TOC_biomass_microwave_microwave_filtered"
...
# import file
# 20ul
df_20ul_1 <- openxlsx::read.xlsx("data/220602_TOC_sample_plate1.xlsx")
...
ggplot(df_plot, aes(x=Dilution_factor, y=TOC_mgml, color=Sample_type, group=Sample_type)) +
  ...
  ggtitle("30s microwaved sample (filtered) \n") +
  # label
...
## No filter method, 30s, 45s

```{r}
# import file
# 20ul
df_20ul_1 <- openxlsx::read.xlsx("data/220602-2_TOC_sample_plate1.xlsx")
df_20ul <- rbind(df_20ul_1)
...
#
df_OD <- openxlsx::read.xlsx('data/220602-2_control_OD600.xlsx')
...
## Last microwave experiment if it does not work
...
# import file
# 20ul
df_20ul_1 <- openxlsx::read.xlsx("data/220602-3_TOC_sample_plate1.xlsx")
df_20ul <- rbind(df_20ul_1)
...
ggplot(df_plot, aes(x=Dilution_factor, y=TOC_mgml, color=Filtered, group=Filtered)) +
  ...
  ylab("Carbon (mg/ml)") +
  xlab("Dilution factor (serial dilution)") +
  scale_y_continuous(breaks = seq(0,0.4,0.05), expand = c(0, 0), limits = c(0, 0.4))+
  ggtitle("30s microwaved sample (filtered) \n") +
  # label
  # geom_text(aes(label = round(NO3_mM,3)), size = 3, vjust = -1.5, family="serif", show.legend = FALSE)+
  mytheme_2d +
...
#
df_OD <- openxlsx::read.xlsx('data/220602-3_control_OD600.xlsx')
df_OD %<>% rename(OD600 = "600") %>% group_by(Dilution_factor) %>% summarize(OD600 = mean(OD600))
df_blank <- df_OD %>% filter(Dilution_factor == 0) %>% unlist()
blank <- df_blank[2]
df_OD %<>% mutate(OD600 = OD600 - blank) %>% filter(Dilution_factor != 0)
...
ggplot(df_plot2, aes(x=OD600, y=TOC_mgml, color=Filtered, group=Filtered)) +
  geom_point(size=3, shape=16) +
  # geom_line(size=1.2)+
  # geom_errorbar(aes(ymin=NO3_mM - Std_NO3_mM, ymax=NO3_mM + Std_NO3_mM), width=.05)+
  # scale_colour_gradientn(colours = col_pH(100)) +
