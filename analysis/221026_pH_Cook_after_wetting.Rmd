---
title: "221026_pH_Cook_after_wetting"
...
## Time series analysis - pH analysis after rewetting
...
```{r}
df_pH1 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_AW_plate1_2_T9_mu_opt_T9_added.xlsx")
df_pH2 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_AW_plate3_4_T9_mu_opt_T9_added.xlsx")
df_pH3 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_AW_plate5_6_T9_mu_opt_T9_added.xlsx")
df_pH4 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_AW_plate7_8_T9_mu_opt_T9_added.xlsx")
df_pH5 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_AW_plate9_10_T9_mu_opt_T9_added.xlsx")
# df_pH6 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_Oxbow3_T9_mu_opt_T9.xlsx") 
# df_pH7 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_Washington2_T0_mu_opt_T0.xlsx")
# df_pH8 <- openxlsx::read.xlsx("data/221026_pH_colorimetric_Washington2_T9_mu_opt_T9.xlsx") 
...
# Filter defect
# I ruined T10....
df_pH3 %<>% filter(!(Well %in% c("F01", "E02"))) # less than 120ul
...
ggplot(df_plot, aes(x=pH_colorimetric, y=pH_meter)) +
  ...
  ggtitle("Comparing pH measuring methods (Water) \n") +
  # label
...
ggplot(df_plot2, aes(x=H_mM, y=pH, color=Soil_name, group=Soil_name)) +
  ...
  # scale_x_continuous(breaks = seq(-100,100,25))+
  scale_y_continuous(breaks = seq(2,11,1), limits=c(2, 11))+
  ggtitle("Soil pH perturbation & titration curves (T9) (After rewetting, Water)") +
  # label
  # geom_text(aes(label = pH_colorimetric), size = 3, vjust = -1.5, family="serif", show.legend  = FALSE)+
  mytheme_2d
  # facet_wrap(. ~ Soil_name, ncol=4) +
...
ggplot(df_plot2, aes(x=H_mM, y=pH, color=Soil_pH, group=Soil_pH)) +
  ...
  # scale_x_continuous(breaks = seq(-100,100,25))+
  scale_y_continuous(breaks = seq(2,11,1), limits=c(2, 11))+
  ggtitle("Soil pH perturbation & titration curves (T9, Water)") +
  # label
  # geom_text(aes(label = pH_colorimetric), size = 3, vjust = -1.5, family="serif", show.legend  = FALSE)+
  mytheme_2d
...
dim(df_pH_export)
write.xlsx(df_pH_export, "221026_pH_Cook_after_wetting.xlsx")

```

## Compare before and after rewetting

```{r}
pH_before <- read.xlsx("data/221016_pH_Cook_before_wetting.xlsx")
pH_after <- read.xlsx("data/221026_pH_Cook_after_wetting.xlsx")
dim(pH_before)
dim(pH_after)

pH_before$Rewetting <- "Before"
pH_after$Rewetting <- "After"

pH_rewet <- rbind(pH_before, pH_after)

head(pH_rewet)
colnames(pH_rewet)

pH_before2 <- pH_before %>% select(Well, Soil, Soil_name, H_mM, pH) %>% rename(pH_before = pH, H_mM_before = H_mM)
pH_after2 <- pH_after %>% select(Well, Soil, Soil_name, H_mM, pH) %>% rename(pH_after = pH, H_mM_after = H_mM)
  
pH_rewet2 <- pH_before2 %>% left_join(pH_after2, by=c("Well"="Well", "Soil"="Soil", "Soil_name"="Soil_name"))

dim(pH_rewet2)
head(pH_rewet2)

# plot scatter plot
ggplot(pH_rewet2, aes(x=pH_before, y=pH_after)) +
  # geom_line(size=1)+
  geom_point(size=3, alpha=1, color="royalblue") +
  # geom_errorbar(aes(ymin=Ave_pH - Std_pH, ymax=Ave_pH + Std_pH), width=.05)+
  geom_abline(intercept=0, slope=1)+
  scale_color_brewer(palette='Set1') +
  ylab("pH after rewetting \n") +
  xlab("\n pH before rewetting") +
  scale_x_continuous(breaks = seq(2,10,1), limits=c(2, 10))+
  scale_y_continuous(breaks = seq(2,10,1), limits=c(2, 10))+
  ggtitle("Comparing pH before and after rewetting (T9, Water) \n") +
  # label
  # geom_text(aes(label = pH_colorimetric), size = 3, vjust = -1.5, family="serif", show.legend  = FALSE)+
  mytheme_2d

# use facet wrap
pH_rewet$Soil_name <-  factor(pH_rewet$Soil_name, levels = vec_soil_name)
pH_rewet$Rewetting <-  factor(pH_rewet$Rewetting, levels = c("Before","After"))

ggplot(pH_rewet, aes(x=H_mM, y=pH, color=Rewetting, group=Rewetting)) +
  geom_point(size=3, shape=16, alpha=1) +
  # geom_errorbar(aes(ymin=Ave_pH - Std_pH, ymax=Ave_pH + Std_pH), width=.05)+
  geom_line(size=1)+
  # scale_color_brewer(palette='Set1') +
  ...
  # scale_x_continuous(breaks = seq(-100,100,25))+
  scale_y_continuous(breaks = seq(2,11,1), limits=c(2, 11))+
  ggtitle("Soil pH titration curves (Before vs After rewetting) \n") +
  # label
  # geom_text(aes(label = pH_colorimetric), size = 3, vjust = -1.5, family="serif", show.legend  = FALSE)+
  mytheme_2d +
  facet_wrap(. ~ Soil_name, ncol=4) +
...
## Starting T9 no acid base and soil pH relationship
pH_rewet %>% filter(Unit == 0)
ggplot(pH_rewet %>% filter(Unit == 0), aes(x=Soil_pH, y=pH, color=Rewetting, group=Rewetting)) +
  geom_point(size=3, shape=16, alpha=1) +
  # geom_errorbar(aes(ymin=Ave_pH - Std_pH, ymax=Ave_pH + Std_pH), width=.05)+
  # geom_line(size=1)+
  # scale_color_brewer(palette='Set1') +
  ...
  ggtitle("[Before vs After] Soil pH & pH at endpoint(T9)") +
  # label
  # geom_text(aes(label = pH_colorimetric), size = 3, vjust = -1.5, family="serif", show.legend  = FALSE)+
  mytheme_2d+
  geom_abline(intercept = 0, slope = 1,
