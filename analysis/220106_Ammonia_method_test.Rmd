---
title: "220106_Ammonia_method_test"
...
## Salicylate-hypochlorite Ammonia assay measurements
Researcher: **Kiseok Lee** \
Experiment Date: 1/6/22 \
Analysis Date: 1/6/22
Lab: **Seppe Kuehn**
...
```{r}
# import file
df_am <- openxlsx::read.xlsx("data/220106_Ammonia_sample_plate1.xlsx")
dim(df_am)
...
ggplot(df_innate, aes(x=Ammonia, y=Ratio_retrieved, fill=Method, group=Method)) +
  ...
  # scale_x_continuous(breaks = seq(0,17,1), limits=c(0, 17))+
  ggtitle("Ammonium-N spike in extraction ratio (Filter vs No filter) \n") +
...
df_filt <- df_am_mcf %>% filter(Method == "Filter")
df_sup <- df_am_mcf %>% filter(Method == "Supernatant")

fit.f <- lm(Ammonia_ppm ~ Ammonia, df_filt)
fit.s <- lm(Ammonia_ppm ~ Ammonia, df_sup)

# (1) Plot fitted linear regression line: filter method
ggplot(df_filt, aes(x=Ammonia, y=Ammonia_ppm)) +
  ...
  # regression line
  geom_abline(slope = coef(fit.f)[[2]], intercept = coef(fit.f)[[1]],
              color = "maroon2") +
  # show equation
  # stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
  annotate("text",x=25,y=15, label= paste0("y = ", round(coef(fit.f)[[1]],3),"+",round(coef(fit.f)[[2]],3),"x"), color = "maroon2") +
  mytheme_2d

# (2) Plot fitted linear regression line: no filter method
ggplot(df_sup, aes(x=Ammonia, y=Ammonia_ppm)) +
  geom_point(size=2.5, shape=21, color = "brown") +
  # geom_line(size=1)+
  # geom_errorbar(aes(ymin=NO3_mM - sd_NO3_mM, ymax=NO3_mM + sd_NO3_mM), width=.05)+
  # scale_color_brewer(palette='Set2') +
  ...
  #scale_y_continuous(breaks = seq(0,0.3,0.05), limits=c(0, 0.3))+
  # geom_text_repel(aes(label = round(NO3_mM,3)), size = 3,family="serif", show.legend = FALSE)+
  ggtitle("Ammonium-N of Midway-A soil (No Filter method) \n") +
  geom_abline(slope = 1, intercept=0, show.legend = "y=x")+
  # regression line
  geom_abline(slope = coef(fit.s)[[2]], intercept = coef(fit.s)[[1]],
              color = "maroon2") +
  # show equation
  # stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
  annotate("text",x=25,y=15, label= paste0("y = ", round(coef(fit.s)[[1]],3),"+",round(coef(fit.s)[[2]],3),"x"), color = "maroon2") +
  mytheme_2d


```

## 3.4. Let's say that innate N is fixed.
...
# innate nitrate
add_innate <- df_ave %>% filter(Ammonia == 0) %>% rename(Innate_Ammonia_ppm = Ave_Ammonia_ppm) %>% select(Method, Innate_Ammonia_ppm)
innate_ammonia <- mean(add_innate$Innate_Ammonia_ppm)

df_innate2 <- df_ave %>% filter(Ammonia != 0) %>% arrange(Sample) %>%
  mutate(Retrieved_Ammonia_ppm = Ave_Ammonia_ppm - innate_ammonia)

# Retrieved ratio of spike ins
df_innate2$Ratio_retrieved <- df_innate2$Retrieved_Ammonia_ppm / df_innate2$Ammonia
df_innate2$Std_Ratio_retrieved <- df_innate2$Std_Ammonia_ppm / df_innate2$Ammonia 

# plot
df_innate2$Ammonia <- factor(df_innate2$Ammonia)

ggplot(df_innate2, aes(x=Ammonia, y=Ratio_retrieved, fill=Method, group=Method)) +
  geom_bar(stat="identity",position="dodge")+
  geom_errorbar(aes(ymin=Ratio_retrieved - Std_Ratio_retrieved, ymax=Ratio_retrieved + Std_Ratio_retrieved), width=.05, position = position_dodge(0.8))+
  # geom_line(size=0.2)+
  scale_fill_brewer(palette='Set2') +
